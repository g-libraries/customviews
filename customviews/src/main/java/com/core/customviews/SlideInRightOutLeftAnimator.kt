...
class SlideInRightOutLeftAnimator(private val mRecyclerView: RecyclerView) :
    DefaultAnimator<SlideInRightOutLeftAnimator>() {
    ...
    private fun retrieveItemPosition(holder: RecyclerView.ViewHolder) {
        mDeltaX =
            (mRecyclerView.width - mRecyclerView.layoutManager!!.getDecoratedLeft(holder.itemView)).toFloat()
    }
